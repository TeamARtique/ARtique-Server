import { NextFunction } from "firebase-functions/node_modules/@types/express";
...
const checkUser = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const decodedToken = await jwtHandlers.verify(accesstoken);
    ...
    // 위의 id 값으로 유저를 조회합니다.
    const user = await userService.findUserById(client, userId);
    // 유저가 없을 시의 에러 처리입니다.
    if (!user)
      return res
        .status(statusCode.UNAUTHORIZED)
